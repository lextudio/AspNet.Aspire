...
<#
  Launch Aspire dashboard with IIS mode, stream logs to the console, detect the "Now listening on:" line,
  and open the default browser to the reported URL when ready.

  Behavior:
  - Checks if running as administrator (required for IIS site creation/management).
  - Sets HOSTING_MODE=IIS environment variable.
  ...
  - On exit, stops the IIS site (best-effort).
#>
...
if (-not $isAdmin) {
	Write-Error "This script requires administrator privileges to manage IIS sites."
...
Write-Output "Starting project in IIS mode: $($projPath.Path)"
...
# Cleanup: stop the IIS site if it was created
try {
	$appcmd = Join-Path $env:SystemRoot 'system32\inetsrv\appcmd.exe'
	if (Test-Path -LiteralPath $appcmd) {
		Write-Output "Stopping IIS site..."
		& $appcmd stop site /site.name:test-aspnet-mvc 2>$null
	}
} catch {
	Write-Warning "Error stopping IIS site: $_"
}
